...
app.get('/api/viagens', async (req, res) => {
    const [rows] = await db.query('SELECT v.*, t.descricao as tipo FROM tbViagem v LEFT JOIN tbViagemTipo t ON v.viagem_tipo_id = t.viagem_tipo_id ORDER BY v.viagem_id DESC');
    res.json(rows);
});
